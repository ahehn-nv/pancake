...
__device__ PacBio::Data::CigarOperationType gw_cigar_to_pb_cigar_op(int8_t action)
{
    ...
    switch (action)
    {
    case AlignmentState::match: return CigarOperationType::SEQUENCE_MATCH;
    case AlignmentState::mismatch: return CigarOperationType::SEQUENCE_MISMATCH;
    case AlignmentState::insertion: return CigarOperationType::INSERTION;
    case AlignmentState::deletion: return CigarOperationType::DELETION;
    default: return CigarOperationType::UNKNOWN_OP;
    };
}

__device__ int64_t compute_score(int8_t action, int32_t runlength, int32_t match_score, int32_t mismatch_penalty, int32_t gap_open_penalty, int32_t gap_ext_penalty)
{
    using claraparabricks::genomeworks::cudaaligner::AlignmentState;
    switch (action) {
...
__global__ void convert_to_pacbio_kernel(PacBio::Data::CigarOperation* cigar, int4* diffs, cuda::atomic<int64_t, cuda::thread_scope_device>* scores, const int64_t* starts, const int32_t* lengths, const int8_t* actions, const int32_t* runlengths, int64_t total_length, int32_t n_alignments, int32_t match_score, int32_t mismatch_penalty, int32_t gap_open_penalty, int32_t gap_ext_penalty)
{
    ...
    if (i < start + len)
    {
        ...
        int64_t score = compute_score(c, rl, match_score, mismatch_penalty, gap_open_penalty, gap_ext_penalty);
...
void RunConvertToPacBioCigarAndScoreGpuKernel(PacBio::Data::CigarOperation* cigar, int4* diffs, int64_t* scores, const claraparabricks::genomeworks::cudaaligner::DeviceAlignmentsPtrs& aln_ptrs, int32_t match_score, int32_t mismatch_score, int32_t gap_open_score, int32_t gap_ext_score, cudaStream_t stream, int32_t device_id)
{
    using claraparabricks::genomeworks::ceiling_divide;
    static_assert(sizeof(int64_t) == sizeof(cuda::atomic<int64_t, cuda::thread_scope_device>), "the cast from int64_t -> cuda::atomic<int64> hack is definitely not supported.");
    ...
    convert_to_pacbio_kernel<<<n_blocks, n_threads, 0, stream>>>(cigar, diffs, reinterpret_cast<cuda::atomic<int64_t, cuda::thread_scope_device>*>(scores) /* please replace by atomic_ref once it is available */, aln_ptrs.starts, aln_ptrs.lengths, aln_ptrs.actions, aln_ptrs.runlengths, total_length, n_alignments, match_score, mismatch_score, gap_open_score, gap_ext_score);
