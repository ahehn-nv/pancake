    ...
    int32_t gap_ext_penalty)
{
    ...
    const int64_t global_start = starts[start_offset_index];
    const int64_t i = global_start + tid;

    // Find alignment index:
    const int32_t offset_idx = upper_bound(starts, starts + n_alignments, i) - starts - 1;
    assert(offset_idx >= start_offset_index);
    if(offset_idx < start_offset_index)
        return;

    const int32_t alignment_idx = indices[offset_idx] >> 1;
    assert(alignment_idx >= 0);
    assert(alignment_idx < n_alignments);
    // Get start and length of the alignment data:
    const int64_t start = starts[offset_idx];
    const int32_t len   = starts[offset_idx+1] - starts[offset_idx];
    ...
    if (i < start + len) {
        ...
        const int8_t c    = actions[start + pos];
        const int32_t rl  = runlengths[start + pos];
        assert(c >= 0);
        assert(c < 4);
        assert(rl >= 0);
        // Translate
    ...
    cudaStream_t stream, int32_t device_id)
{
    ...
    convert_to_pacbio_kernel<<<n_blocks, n_threads, 0, stream>>>(
        ...
        aln_ptrs.starts, aln_ptrs.lengths, aln_ptrs.actions, aln_ptrs.runlengths,
