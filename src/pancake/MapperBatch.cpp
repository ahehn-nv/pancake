    ...
    MapperCLRSettings settings, int32_t numThreads)
{
    ...
    if (settings.align) {
        ...
        AlignerBatchCPU aligner(alignerGlobal, alignerExt);

        ///// Prepare the sequences and add them to the aligner.
        // Results are a vector for every chunk (one chunk is one ZMW).
        for (size_t resultId = 0; resultId < results.size(); ++resultId) {
            auto& result = results[resultId];
            ...
            for (size_t qId = 0; qId < result.size(); ++qId) {
                ...
                const std::string qSeqRevString =
                    PacBio::Pancake::ReverseComplement(qSeqFwd, 0, qLen);
