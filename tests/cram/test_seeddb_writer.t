...
Build the SeedDB from a SeqDB which is split into multiple blocks, and split the seeds into multiple files.
  $ rm -f test-11*
  > ${BIN_DIR}/pancake seeddb --split-blocks -k 30 -w 80 ${PROJECT_DIR}/test-data/seqdb-writer/test-11-split-blocks.seqdb test-11b-split-blocks
...
Build the SeedDB from a SeqDB which is split into multiple blocks, but keeps the seeds in a single file.
  $ rm -f test-11*
  > ${BIN_DIR}/pancake seeddb -k 30 -w 80 ${PROJECT_DIR}/test-data/seqdb-writer/test-11-split-blocks.seqdb test-11a-single-file
