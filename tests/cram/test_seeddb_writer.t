...
Test construction of the DB from a small test FASTA file. Store each sequence into a separate 2-bit compressed file.
  $ ${BIN_DIR}/pancake seeddb -k 30 -w 80 --split-blocks --buffer-size 0.001 ${PROJECT_DIR}/test-data/seqdb-writer/test-1.seqdb test-1b
...
Test construction of the DB from a small test FASTA file. Store each sequence into a separate 2-bit compressed file.
  $ ${BIN_DIR}/pancake seeddb -k 30 -w 80 --split-blocks --buffer-size 0.011 ${PROJECT_DIR}/test-data/seqdb-writer/test-1.seqdb test-1c
  > diff ${PROJECT_DIR}/test-data/seeddb-writer/test-1c.seeddb test-1c.seeddb
  > diff ${PROJECT_DIR}/test-data/seeddb-writer/test-1c.seeddb.0.seeds test-1c.seeddb.0.seeds
  > diff ${PROJECT_DIR}/test-data/seeddb-writer/test-1c.seeddb.1.seeds test-1c.seeddb.1.seeds
